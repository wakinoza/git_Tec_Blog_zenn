...
しかし、このままでは、GitHubでソースを共有したメンバーには「.env」ファイルに何の項目を定義すればよいのかという情報も伝わりません。別の環境でアプリを実行するために、コードを確認し、どの項目が観光変数として「.env」に記載する必要があるのかを一から調べるのは大変な手間です。

そこで、「.env」のひな型である「.env.example」を作成し、Gitに管理し、GitHubで共有します。
「.env.example」は、環境変数の項目だけを字最多ファイルです。項目だけで、値は記載されていないため、機密情報は含まれません。
