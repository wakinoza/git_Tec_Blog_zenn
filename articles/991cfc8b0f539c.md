...
これで、機密情報をGit管理外とすることができました。
しかし、このままでは、GitHubでソースを共有したメンバーには「.env」ファイルに何の項目を定義すればよいのかという情報も伝わりません。別の環境でアプリを実行するために、コードを確認し、どの項目が環境変数として「.env」に記載する必要があるのかを一から調べるのは大変な手間です。

そこで、「.env」のひな型である「.env.example」を作成し、Gitに管理し、GitHubで共有します。
「.env.example」は、環境変数の項目だけを記述したファイルです。項目だけで、値は記載されていないため、機密情報は含まれません。
...
GitHubでアプリ情報を共有したメンバーは、「.env.example」をコピーし、それぞれの環境用の「.env」を作成することで、手軽にアプリを実行します。

「.env」「.env.example」を利用することで、機密情報の漏洩を防ぎつつ、アプリ情報をメンバーで共有することができるのです。
