...
Stringクラスは、不変のイミュータブルなクラスです。そのため、一度インスタンスが生成されたら、その内容を後から変更することはできません。String型変数へ値を再代入した場合は、変数の指し示すインスタンスが変化するのではなく、変数の中身が新しいインスタンスへの参照に置き換わっています。
具体例を挙げて説明しましょう。
...
②で、変数bに、変数aが持つ"abcde"のインスタンスへの参照がコピーされました。
その上で、③のように変数bに別の値を代入すると、どうなるでしょう？

"aiueo"を代入すると、内部では新しい文字列"aiueo"のインスタンスが自動的に生成されます。それにより、変数bの"abcde"への参照が切れて、"aiueo"への参照に置き換わります。

では、変数aはどうなっているでしょう？
多くの参照型変数では、コピー先で内容を変更すると、コピー元の内容も変更されますが、Stringクラスの挙動は異なります。

Stringクラスでは、再代入されても、新しいインスタンスが生成されるだけで、元のインスタンスに変更はありません。そのため、3の代入が完了した後も、文字列"abcde"のインスタンスは変わらず存在します。変数aにも変更がないため、変数aも変わらず文字列"abcde"のインスタンスへの参照を格納しています。
不変であるStringクラスでは、コピー先で内容を変更しても、コピー元の内容は変わらないのです。

Stringクラスはコード中で多用されるため、コピー先で意図せず内容が書き換えられてしまうと、バグや思わぬ挙動の原因となります。そのため、Stringクラスはイミュータブルという特徴を持っているのです。
...
- 引数が負の場合は、IllegalArgumentExceptionとなる
...
- Stringクラスはイミュータブルであるため、コピー先で内容が変更されても、コピー元は変化しない
