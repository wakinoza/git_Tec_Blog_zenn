...
Stringクラスは参照型ですが、イミュータブルという性質があるため、他の参照型とは異なる挙動をします。
具体例を挙げてみていきましょう。
...
String b = a;          //②
b = "aiueo";           //③
System.out.println(b);  //結果：aiueo
System.out.println(a);  //結果：abcde
```
...
Stringクラスでは、変数に再代入されても、新しいインスタンスが生成されるだけで、元のインスタンスに変更はありません。そのため、③の後も、文字列"abcde"のインスタンスは変わりません。変数aも変更がないため、変数aは変わらず文字列"abcde"のインスタンスへの参照を格納しています。

配列など多くの参照型変数では、参照のコピー先での内容変更がコピー元にも広がり、バグや思わぬ挙動の原因となります。
影響拡大を防ぐためにはインスタンスをクローンするなどの記述が必要ですが、参照をコピーするたびにクローンを作成するとコードが冗長になります。

しかし、Stringクラスはイミュータブルという性質を持っているため、クローンを作らなくでも、影響がコピー元まで広がることはありません。

Stringクラスは、イミュータブルという性質によって、コードを簡素にし、バグや思わぬ挙動を防いでいるのです。
