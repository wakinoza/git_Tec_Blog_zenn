...
では、なぜLinuxのファイル管理において、このようなinodeの仕組みが採用されているのでしょうか？

Linuxがこの戦略を採る理由は主に2つあります。
...
Windowsのファイルシステム（NTFS）などでは、MFT（Master File Table）という領域でファイル名とメタデータをまとめて管理する異なる形態をとっており、inodeはあくまでLinuxやUNIX系システムが採用しているファイル管理の「戦略」です。
...
Linuxにおいて、ディレクトリ内にある「ファイル名」と「inode番号」の対応付け（エントリ）そのものを「ハードリンク」と呼びます。
そのため、別名を持たない通常のファイルであっても、作成された時点で「1つのハードリンク（対応付け）」がすでに存在しています。

そしてLinuxのシステムでは、1つのファイル実体（inode）に対して、複数のファイル名（ハードリンク）を重ねて対応付けることが可能です。
結果として、「複数の異なるファイル名から、同一のinode（およびデータ本体）を共有・参照する」という機能を実現しています。
...
ハードリンクを作成する場合は、以下のコマンドを実行してください。
...
その理由は複合的ですが、最大の理由はファイルシステムの整合性と管理構造が破綻するのを防ぐためです。

もしディレクトリのハードリンク（別名）を許してしまうと、「親から子へ」というディレクトリツリーの一意性が崩れ、ファイルシステム内に循環構造（ループ）が発生する可能性があります。
また、パーミッション（アクセス制限ポリシー）は上位ディレクトリから順に適用されるため、複数のハードリンクが存在すると、パーミッションの抜け穴が生じる危険性が跳ね上がります。

さらに、ハードリンクが可能になると、ディレクトリの削除・アンリンク処理が複雑化します。
通常のファイルは、名前を削除する操作（アンリンク）によってリンク数を減らし、0になったら実体を消去します。しかしディレクトリは、配下にさらに子ファイルや子ディレクトリを内包しています。もしディレクトリに複数経路のハードリンク（別名A・B）を許してしまうと、Aの名前を削除した際、残ったBのルートから見た配下の子ファイルたちの階層構造（ツリーの一意性）に矛盾が生じる可能性があります。この矛盾を解消するには、ディレクトリ構造の精査・再計算が必要になりますが、それらは複雑な処理となるため、現実的ではありません。
このような問題があるため、ディレクトリのハードリンクができない仕様となっているのです。
...
シンボリックリンクを作成する際は、「ln」コマンドに「-s」オプションを指定して実行してください。
...
では、この状態で元ファイルを削除するとどうなるでしょうか？
元ファイルを削除し、シンボリックリンクファイルを読み出してみましょう。
以下のコマンドを実行してください。
